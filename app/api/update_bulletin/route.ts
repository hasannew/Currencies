...
async function handler(req: NextRequest) {
  const { storeName, state, city, currencies,bulletinID } = await req.json();
  console.log(storeName);
  console.log(state);
  console.log(city);
  if (!storeName || !state || !city || !bulletinID || currencies.length == 0) {
  ...
  const refcite = await db.bulletin.update({
    where:{
        id:bulletinID
    },
    data: {
      ...
      products: { 
        set: [],
        connect: products.map((currency) => ({ id: currency.id })) },
    },
  });
